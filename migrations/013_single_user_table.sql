...
--
-- Migration 012 made User_Account a supertype with Student and Admin hanging
-- off it. Now that roles carry every access decision, nothing needs to know
-- which kind of row a user is — one table and User_Role answer everything.
--
-- It also unblocks a single login page: requireAuth looked users up in Student,
-- where an administrator simply was not, so the two needed separate flows.
--
...
-- inserted at the ids 012 gave them, keeping their User_Role grants valid.
--
...
-- would read as the RBAC role, which it is not — User_Role is.
--
-- Wrapped in a procedure so re-running migrate.sh does nothing.
--
...
BEGIN
    ...
    -- 1. Park the administrator rows with the user_id 012 gave them. A real
    --    table, not TEMPORARY: some clients drop those between statements.
    ...
    -- 2. Release every foreign key pointing at the 012 User_Account so it can
    --    be dropped and the name freed for the renamed Student table.
    ...
    -- 3. Student becomes the one user table. MySQL rewrites the six foreign
    --    keys referencing it as part of the rename.
    ...
    --    rows 012 created still point at the right person. is_active collapses
    --    into account_status: both answered the same question, one checked by
    ...
    -- 7. The audit log recorded admin_id. Translate to user_id before Admin
    --    goes, or every historical row loses its actor.
