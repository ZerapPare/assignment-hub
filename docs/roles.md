...
> **สถานะ:** `init.sql`, `migrations/` และโค้ดฝั่ง backend seed บทบาทไว้ 2 ตัวตามเอกสารนี้แล้ว
> สิ่งที่ยังไม่ได้ทำคือข้อบังคับ **1 คน 1 บทบาท** — `User_Role` ยังใช้ `PRIMARY KEY (user_id, role_id)`
> ซึ่งยอมให้ถือหลายบทบาทได้ และยังไม่มี `requireStudent` กัน admin ออกจากหน้านักศึกษา
