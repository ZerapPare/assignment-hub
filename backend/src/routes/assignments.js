...
router.get('/api/assignments', requireAuth, async (req, res) => {
	try {
		const onlyActive = req.query.filter === 'active';

		const where = onlyActive
			? `WHERE c.student_id = ?
         AND (d.status IS NULL OR d.status NOT IN ('submitted', 'completed'))`
			: `WHERE c.student_id = ?`;

		const [rows] = await pool.query(
			`${ASSIGNMENT_SELECT} ${where} ORDER BY d.due_date IS NULL, d.due_date ASC`,
			[req.session.userId]
		);
		res.json(rows);
	} catch (err) {
